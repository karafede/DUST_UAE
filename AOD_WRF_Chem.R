...
# load WRFChem output from Federico (Extinction coeff for DUST simulation ,2 April 2015 dust storm)
filenames <- "EXTCOEF55_DUST_3.nc"  # small domain
...
#### only one variable 
     ...
     var_value<-WRF_file[14]        # extcof55_fk == var = 14
     z_e <- WRF_file[13]            # geopotential height z_e == var == 13  in meters 
 ...
 # get altitude levels (from z_e) and compute steps
    ...
    for (i in 1:(dim(z_e)[3]-1)){    # eta (height, 44 levels)
       ...
       # else 
       #   dz <- (z_e[ , ,44, ] - z_e[ , , 43, ])*0.001   # height levels (km)
...
# j = 6  
  ...
  for (j in 1:dim(var_value)[4]){      # time dimension (always 115)
...
# kk <- 50
 ...
 for (kk in 1:115){          # number of hours (time stamp), 115 hours
   plot(BBB[[1]],kk)
...
writeRaster(ras_stack, "AOD_WRFChem_02April2015_stack_5_DAYS.tif" , options= "INTERLEAVE=BAND", overwrite=T)
...
AOD_images <- stack("D:/Dust_Event_UAE_2015/WRF_trial_runs/AOD_WRFChem_02April2015_stack_5_DAYS.tif")

for (i in 1:length(AOD_images@layers)) {
TITLE <- paste(TS[i], " (UTC)")
name_time <- TS[i]
AOD_images <- raster("D:/Dust_Event_UAE_2015/WRF_trial_runs/AOD_WRFChem_02April2015_stack_5_DAYS.tif", band = i)
# plot(AOD_images)
